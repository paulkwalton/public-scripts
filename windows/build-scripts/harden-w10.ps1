# This is designed to build a Windows 10 Host laptop, only with Vmware Workstation. The base OS should be kept minimal with reduced attack surface.
# Install a Windows 10 Pro Image using the standard ISO file from Microsoft, and run this script.
...
# Disable Hyper V to stop any conflicts with VMware Workstation
Disable-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V-Hypervisor

# Enable Windows Firewall by default. You can add exceptions if needed to reverse shells and pentest traffic from Vmware.
...
# Download Hashcat
Invoke-WebRequest -Uri "https://hashcat.net/files/hashcat-6.2.6.7z" -OutFile "c:\tools\wordlists\hashcat-6.2.6.7z"

# Install Basic Tools For a Host OS running VMware Workstation
choco install bginfo -y
choco install notepadplusplus.install -y
choco install 7zip.install -y

# Git clone further wordlists for hashcat
git clone https://github.com/danielmiessler/SecLists.git c:\tools\


#The below code is taken from the excellent script https://github.com/Sycnex/Windows10Debloater. Credit to Sycnex to save me reinventing the wheel.
